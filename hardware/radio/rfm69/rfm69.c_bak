...
uint8_t
rfm_cmd(uint16_t command, uint8_t wnr)
{

  // Split command in two bytes, merge with write-/read-flag
  uint8_t highbyte =
  (wnr ? ((command >> 8) | 0x80) : ((command >> 8) & 0x7F));
...
// slow spi
   ...
   * 1   0   fosc/64
   * 1   1   fosc/128
   * detailed setting still ignored
   * will use code from rfm12 fro now
   */

#ifdef CONF_RFM69_SLOW_SPI
 ...
 */
static uint8_t
rfm_fifo_wnr(char *data, uint8_t wnr)
{
  ACTIVATE_RFM69;
